    ...
    <FadeSection id="pricing">
      ...
      <section className="bg-bg-900 py-20 px-6">
        <div className="text-center mb-12">
          ...
          <h2 className="font-body font-bold text-[32px] md:text-[40px] tracking-[-0.5px] text-[#F1F5F9]">무료로 시작하고, 확신이 생기면 Pro로</h2>
        </div>
        <div className="max-w-[800px] mx-auto">
          ...
          <p className="text-center mt-3 text-[13px] text-[#64748B]">유료 전용 경기는 결과만 다음날 공개됩니다</p>
        </div>
        <div className="max-w-[780px] mx-auto mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-bg-800 border border-bg-700 rounded-2xl px-7 py-9 card-hover">
            ...
            <div className="font-outfit font-[800] text-[42px] tracking-[-2px] text-[#F1F5F9]">0<small className="text-base font-medium text-[#94A3B8] font-body">원/월</small></div>
            <div className="text-sm text-[#94A3B8] mt-1">카톡 채널 추가만 하면</div>
            <ul className="mt-5 mb-6 space-y-1.5">
              {FREE_FEATURES.map((f) => <li key={f} className="text-sm flex items-center gap-2 text-[#94A3B8]"><span className="text-emerald-500 font-bold">✓</span> {f}</li>)}
            </ul>
            <a href={KAKAO_CHANNEL_URL} target="_blank" rel="noopener noreferrer"
              className="block w-full py-3.5 rounded-lg text-[15px] font-bold text-center bg-transparent border border-bg-700 text-[#94A3B8] hover:border-emerald-500 hover:text-emerald-500 transition-all">무료로 시작하기</a>
          </div>
          <div className="relative bg-bg-800 border-2 border-emerald-500 rounded-2xl px-7 py-9 neon-card card-hover">
            <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-emerald-500 text-white text-xs font-bold px-4 py-1 rounded-full">추천</div>
            <div className="font-body font-bold text-xl mb-1 text-[#F1F5F9]">Pro</div>
            <div className="text-sm text-[#64748B] line-through">14,900원</div>
            <div className="font-outfit font-[800] text-[42px] tracking-[-2px] gold-shimmer">
              {EARLYBIRD_PRICE.toLocaleString()}<small className="text-base font-medium text-[#94A3B8] font-body" style={{WebkitTextFillColor:"#94A3B8",background:"none"}}>원/월</small>
            </div>
            <div className="inline-block mt-1.5 mb-5 bg-gold-400/10 text-gold-400 border border-gold-400/50 text-[13px] font-semibold px-3 py-1 rounded-full earlybird-glow">
              얼리버드 — 커피 3잔 가격
            </div>
            <ul className="mb-6 space-y-1.5">
              {PRO_FEATURES.map((f) => <li key={f} className="text-sm flex items-center gap-2 text-[#94A3B8]"><span className="text-emerald-500 font-bold">✓</span> {f}</li>)}
            </ul>
            <a href="/login?redirect=/pricing" className="block w-full py-3.5 rounded-lg text-[15px] font-bold text-center bg-emerald-500 hover:bg-emerald-700 text-white transition-colors">Pro 시작하기</a>
          </div>
        </div>
      </section>
    </FadeSection>
