...
function BarRow({ label, rate, color, delay, animate }) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-12 text-[13px] font-medium text-right shrink-0 text-[#94A3B8] font-body">{label}</div>
    ...
    <FadeSection id="dashboard">
      ...
      <section className="bg-bg-900 py-20 px-6">
        <div className="text-center mb-4">
          ...
          <h2 className="font-body font-bold text-[28px] md:text-[36px] tracking-[-0.5px] leading-[1.2] text-[#F1F5F9]">적중률 투명 공개</h2>
          <p className="text-[#94A3B8] mt-2 text-[15px] font-body">모든 예측과 결과를 기록합니다. 숨기지 않습니다.</p>
        </div>
        <div className="max-w-[900px] mx-auto">
          <div className="flex justify-center gap-2 mb-8">
            {TABS.map((t) => <button key={t} onClick={() => setTab(t)} className={`px-4 py-1.5 rounded-full text-[13px] font-bold cursor-pointer transition-all font-body ${tab === t ? "bg-emerald-500 text-white" : "bg-bg-800 border border-bg-700 text-[#94A3B8]"}`}>{t}</button>)}
          </div>
