...
const STEPS = [
  {
    ...
    mock: (
      <div className="bg-bg-800 border border-bg-700 rounded-lg p-3 text-sm mt-4">
        <div className="font-bold text-[#E1E7EF] text-xs mb-1.5">MATCHLAB AI 분석</div>
        <div className="text-[#E1E7EF] text-xs font-semibold">아스널 vs 첼시</div>
        <div className="text-[11px] text-[#8494A7] mt-0.5">
  ...
  },
  {
    ...
    mock: (
      <div className="bg-bg-800 border border-bg-700 rounded-lg p-3 text-sm font-mono-data mt-4">
        <div className="text-[10px] text-[#566378] mb-1.5 truncate">matchlab.vercel.app/pro/2026-03-28</div>
        <div className="border-t border-bg-700 pt-1.5">
          <div className="text-[11px] text-[#E1E7EF] font-bold mb-1">오늘의 전경기 AI 분석</div>
          <div className="text-[10px] text-gold-400">★5 나폴리 vs 유벤투스 — 78.2%</div>
          <div className="text-[10px] text-gold-500">★4 아스널 vs 첼시 — 62.3%</div>
          <div className="text-[10px] text-[#566378]">... 외 10경기</div>
        </div>
      </div>
    ),
  },
  {
    ...
    mock: (
      <div className="bg-bg-800 border border-bg-700 rounded-lg p-3 text-sm mt-4">
        <div className="text-[11px] text-[#8494A7] font-semibold mb-1.5">어제 결과</div>
        ...
        <div className="text-[10px] text-[#8494A7] mt-1.5 font-semibold">골드 경기 2/3 적중 (66.7%)</div>
      </div>
    ),
  },
];
    ...
    <FadeSection>
      <section className="bg-bg-900 py-20 px-6">
        <div className="text-center mb-12">
          ...
          <h2 className="font-body font-bold text-[32px] md:text-[40px] tracking-[-0.5px] text-[#E1E7EF]">카톡에서 시작, 웹에서 전부 확인</h2>
        </div>
        <div ref={ref} className="max-w-[900px] mx-auto flex flex-col md:flex-row gap-4 items-stretch">
          {STEPS.map((s, i) => (
            <div key={s.step} className="contents">
              <div
                className={`flex-1 bg-bg-800 border border-bg-700 rounded-[14px] p-6 flex flex-col card-hover transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              ...
              >
                ...
                <p className="text-sm text-[#8494A7] leading-relaxed">{s.desc}</p>
                {s.mock}
              </div>
              {i < STEPS.length - 1 && (
