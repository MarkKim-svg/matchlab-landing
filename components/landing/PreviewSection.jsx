...
const MATCHES = [
  { confLabel:"★5", confClass:"bg-gold-400 text-bg-900 star-glow", teams:"나폴리 vs 유벤투스", meta:"세리에A · 04:00",
  ...
  return (
    <div className="border border-bg-700 rounded-lg px-3 py-2.5 mb-2 flex gap-3 items-center card-hover bg-bg-800">
      ...
      <div className="min-w-0 flex-1">
        <div className="text-[13px] font-bold text-[#F1F5F9]">{match.teams}</div>
        <div className="text-[10px] text-[#64748B]">{match.meta}</div>
        {match.locked
          ? <div className="text-[11px] mt-0.5 text-pro font-semibold">Pro 전용</div>
          : <div className="text-[11px] mt-0.5 text-[#94A3B8]">AI: <strong className="text-emerald-400">{match.pred}</strong></div>}
      </div>
      {!match.locked && match.stats && (
        <div className="hidden md:flex flex-col gap-px text-right shrink-0">
          {match.stats.map((s) => <div key={s.label} className="text-[9px] text-[#64748B] font-mono-data">{s.label} <span className="font-semibold text-[#94A3B8]">{s.value}</span></div>)}
        </div>
      )}
    </div>
  );
    ...
    <FadeSection>
      ...
      <section className="bg-bg-900 py-20 px-6">
        <div className="text-center mb-12">
          ...
          <h2 className="font-body font-bold text-[32px] md:text-[40px] tracking-[-0.5px] text-[#F1F5F9]">매일 아침 이런 분석이 도착합니다</h2>
        </div>
        <div className="max-w-[1000px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          <div>
            <div className="font-mono-data font-medium text-[11px] tracking-[0.25em] uppercase text-[#94A3B8] mb-3 text-center">무료 카톡 프리뷰</div>
          ...
          <div>
            ...
            <div className="bg-bg-800 border border-bg-700 border-b-0 rounded-t-xl px-4 py-2.5 flex justify-between items-center">
              <div className="bg-bg-900 rounded-md px-3 py-1 text-[10px] text-[#64748B] font-mono-data">matchlab.vercel.app/pro</div>
              <div className="bg-pro text-white text-[10px] font-bold px-2.5 py-0.5 rounded-full">Pro 전용</div>
            </div>
            <div className="bg-bg-800 rounded-b-xl border border-bg-700 border-t-0 p-4 neon-card">
              <div className="font-body font-bold text-[15px] text-[#F1F5F9] mb-3">오늘의 전경기 AI 분석</div>
              {MATCHES.map((m) => <MatchCard key={m.teams} match={m} />)}
              <div className="text-center pt-3 pb-1">
                <p className="text-[13px] text-[#94A3B8] mb-3">오늘 분석 12경기 중 2경기만 공개 중</p>
                <a href="#pricing" className="inline-flex bg-emerald-500 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-lg text-sm font-bold transition-colors">Pro 시작하기 →</a>
              </div>
            </div>
          </div>
        </div>
