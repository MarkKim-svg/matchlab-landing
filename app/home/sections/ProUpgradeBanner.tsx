...
export default function ProUpgradeBanner({ dashboard, loading }: Props) {
  ...
  return (
    <div
      className="mx-4 my-4 rounded-2xl p-5 text-center"
      style={{
        background: "linear-gradient(135deg, #1c1308, #2a1a08)",
        border: "1px solid #d97706",
      }}
    >
      <div className="text-[16px] font-bold mb-3" style={{ color: "#FBBF24" }}>
        🔓 고확신 경기를 놓치고 있어요
      </div>

      {hasData ? (
        <div className="font-mono-data text-[22px] font-bold mb-3" style={{ color: "#FBBF24" }}>
          {hc.correct}/{hc.total} 적중
        </div>
      ) : (
        <div className="text-[14px] mb-3" style={{ color: "#d4a373" }}>
          고확신 경기를 받아보세요
        </div>
      )}

      <div className="text-[13px] mb-4 leading-[1.5]" style={{ color: "#d4a373" }}>
      ...
      <a
        ...
        className="inline-block text-white font-bold rounded-xl px-7 py-3 text-[14px]"
        style={{
