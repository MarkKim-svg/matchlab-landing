...
const CONF_BARS = [
  ...
  { label:"2성", rate:41, color:"bg-[#566378]" },
...
function BarRow({ label, rate, color, delay, animate }) {
  return (
    <div className="flex items-center gap-3">
      <div className="w-12 text-[13px] font-medium text-right shrink-0 text-[#8494A7] font-body">{label}</div>
      <div className="flex-1 h-7 bg-bg-900 rounded-md overflow-hidden">
        <div className={`h-full rounded-md flex items-center justify-end pr-2.5 text-xs font-medium text-white font-mono-data tracking-[-0.5px] min-w-[36px] ${color} ${animate ? "bar-animate" : ""}`} style={{ width:`${rate}%`, animationDelay:`${delay}ms` }}>{rate}%</div>
      </div>
    </div>
  );
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(([e]) => { if (e.isIntersecting) setBarVis(true); }, { threshold: 0.2 });
    ...
    <FadeSection id="dashboard">
      ...
      <section className="bg-bg-900 py-20 px-6">
        <div className="text-center mb-4">
          ...
          <h2 className="font-body font-bold text-[28px] md:text-[36px] tracking-[-0.5px] leading-[1.2] text-[#E1E7EF]">적중률 투명 공개</h2>
          <p className="text-[#8494A7] mt-2 text-[15px] font-body">모든 예측과 결과를 기록합니다. 숨기지 않습니다.</p>
        </div>
        <div className="max-w-[900px] mx-auto">
          <div className="flex justify-center gap-2 mb-8">
            {TABS.map((t) => <button key={t} onClick={() => setTab(t)} className={`px-4 py-1.5 rounded-full text-[13px] font-bold cursor-pointer transition-all font-body ${tab === t ? "bg-emerald-500 text-white" : "bg-bg-800 border border-bg-700 text-[#8494A7]"}`}>{t}</button>)}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            {STATS.map((s, i) => (
              <div key={s.label} className="bg-bg-800 border border-bg-700 rounded-[14px] p-5 text-center card-hover" style={{transitionDelay:`${i*150}ms`}}>
                <div className={`font-display font-bold text-[40px] tracking-[-2px] ${s.shimmer ? "text-gold-400" : "text-[#E1E7EF]"}`}>{s.value}</div>
                <div className="text-[13px] text-[#566378] mt-1 font-body">{s.label}</div>
              </div>
            ))}
          </div>
          <div ref={barRef} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-bg-800 border border-bg-700 rounded-[14px] p-7">
              <div className="font-body font-bold text-base mb-5 text-[#E1E7EF]">확신도별 적중률</div>
              <div className="flex flex-col gap-2.5">{CONF_BARS.map((b, i) => <BarRow key={b.label} {...b} delay={i*150} animate={barVis} />)}</div>
            </div>
            <div className="bg-bg-800 border border-bg-700 rounded-[14px] p-7">
              <div className="font-body font-bold text-base mb-5 text-[#E1E7EF]">리그별 적중률</div>
