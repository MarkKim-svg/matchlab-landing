        ...
        {loading ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: "12px" }}>
            {Array.from({ length: 12 }, (_, i) => (
              <div key={i} className="animate-pulse" style={{ height: "120px", borderRadius: "14px", background: "#1A2332" }} />
            ))}
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: "12px" }}>
            {teams.map(t => (
              <Link key={t.teamId} href={`/team/${t.teamId}?league=${leagueId}`}>
                <div style={{ background: "#111827", border: "1px solid #1E2D47", borderRadius: "14px", padding: "16px 12px", textAlign: "center", cursor: "pointer" }} className="hover:border-emerald-500/30 transition-colors">
