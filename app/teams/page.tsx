...
interface Scorer { rank: number; playerName: string; playerPhoto: string; teamName: string; teamLogo: string; goals: number; assists: number; }
...
export default function TeamsPage() {
  ...
  const sortedScorers = useMemo(() => {
    let list = [...scorers];
    if (search) list = list.filter(s => s.playerName.toLowerCase().includes(search.toLowerCase()));
    list.sort((a, b) => {
    ...
    <div style={{ minHeight: "100vh", background: "#0F172A" }}>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-6 pb-24 md:pb-8">
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
          {([{ key: "teams" as const, label: "⚽ 팀" }, { key: "players" as const, label: "👤 선수" }]).map(t => (
        ...
        {/* League tabs */}
        <div style={{ display: "flex", gap: "8px", overflowX: "auto", marginBottom: "16px" }} className="scrollbar-hide">
          {LEAGUES.map(l => (
        ...
        {tab === "players" && (
          <>
            ...
            <input
              ...
              style={{ width: "100%", marginBottom: "12px", padding: "10px 14px", borderRadius: "10px", border: "1px solid #334155", background: "#111827", color: "#E1E7EF", fontSize: "13px", outline: "none" }}
            />

            {/* Sub tabs */}
            <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
              {([{ key: "goals" as const, label: "득점" }, { key: "assists" as const, label: "도움" }, { key: "points" as const, label: "공격P" }]).map(t => (
            ...
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "40px 0", color: "#566378" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                {sortedScorers.map((s) => {
                  ...
                  return (
                    <Link key={`${s.playerName}-${s.rank}`} href={`/player/0`} style={{ textDecoration: "none" }}>
                      <div style={{ background: medal ? `${medal}10` : "#111827", border: medal ? `1px solid ${medal}30` : "1px solid #1E2D47", borderRadius: "10px", padding: "10px 14px", display: "flex", alignItems: "center", gap: "10px" }} className="hover:border-emerald-500/30 transition-colors">
                        <span style={{ fontSize: "12px", fontWeight: 700, color: medal ?? "#566378", fontFamily: "'JetBrains Mono',monospace", width: "24px", textAlign: "center", flexShrink: 0 }}>{s.rank}</span>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: "13px", fontWeight: 600, color: medal ?? "#E1E7EF", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.playerName}</div>
