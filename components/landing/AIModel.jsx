    ...
    <FadeSection>
      <section className="bg-bg-900 py-20 px-6">
        <div className="text-center mb-12">
          ...
          <h2 className="text-[32px] font-bold tracking-[-0.02em] text-[#F1F5F9]">
            4개의 모델이 실험합니다
          </h2>
        </div>
        ...
        <div className="max-w-[900px] mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {MODELS.map((m) => (
              <div key={m.name} className="bg-bg-800 border border-bg-700 rounded-2xl p-5 text-center card-hover">
                ...
                <h3 className="text-sm font-bold text-[#F1F5F9] mb-1">{m.name}</h3>
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="bg-bg-800 border border-emerald-500/30 rounded-xl px-6 py-3 text-center">
              <div className="text-xs text-[#64748B] mb-1">앙상블 조합</div>
              <div className="text-sm font-bold text-emerald-400">Claude AI가 최종 확신도를 결정</div>
            </div>
            <div className="w-px h-8 border-l border-dashed border-bg-700" />
            <div className="text-[13px] text-[#94A3B8]">→ 카카오톡으로 발송</div>
          </div>
        </div>
      </section>
    </FadeSection>
