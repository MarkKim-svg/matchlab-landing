...
function ResultCard({ r, index, visible }) {
  return (
    <div className={`bg-bg-800 border ${r.cardBorder} rounded-[14px] p-6 card-hover transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      style={{transitionDelay:`${index*150}ms`}}>
      <div className="flex items-center gap-2 mb-4">
        <span className={`${r.badgeClass} font-bold rounded-full px-3 py-1 text-sm font-display`}>{r.badge}</span>
        {r.hit && <span className="text-gold-400 text-xs font-bold text-gold-400">적중</span>}
      </div>
      ...
      {r.hit ? <div className="text-gold-400 font-bold text-gold-400">✅ 적중</div> : (
    ...
    <FadeSection>
      ...
      <section className="bg-bg-900 py-20 md:py-24 px-6">
        ...
        <div className="text-center mt-8">
          <span className="font-mono-data font-medium text-sm text-gold-400">3월 4주차: 고확신(★4+) 경기 8건 중 6건 적중 — 75.0%</span>
        </div>
      </section>
    </FadeSection>
  );
