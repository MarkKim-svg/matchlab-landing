...
export default function Hero() {
  ...
  return (
    <section className="bg-bg-900 lab-grid px-6 py-24 md:py-32 relative overflow-hidden">
      {BUBBLES.map((b, i) => <div key={i} className={`bubble ${b.w} ${b.bg} ${b.pos}`} style={{animation:`${b.anim} ease-in-out infinite`}} />)}
      <div className="max-w-[900px] mx-auto text-center relative z-10">
        ...
        <div className="mb-6">
          <div className="font-body font-normal text-lg text-[#64748B] mb-3 tracking-[0.5px]">매일 아침,</div>
          <div className="font-body font-bold text-[32px] md:text-[40px] leading-[1.15] tracking-[-0.5px]">
            <span className="text-emerald-400 emerald-glow">AI가 골라주는</span>{" "}
            <span className="text-[#F1F5F9]">오늘의 경기</span>
          </div>
        </div>

        <p className="font-body font-normal text-[15px] text-[#94A3B8] max-w-[560px] mx-auto mb-10 break-keep leading-[1.7]">
        ...
        <div className="flex flex-wrap gap-3 justify-center mb-6">
          ...
          <button onClick={() => document.getElementById("dashboard")?.scrollIntoView({ behavior: "smooth" })}
            className={`bg-transparent border border-[#F1F5F9]/30 text-[#F1F5F9] text-[15px] font-semibold px-8 py-3.5 rounded-lg hover:bg-white/10 transition-all cursor-pointer font-body ${bouncing ? "animate-[subtle-bounce_0.6s_ease-in-out]" : ""}`}>
            적중률 보기
          </button>
        </div>
